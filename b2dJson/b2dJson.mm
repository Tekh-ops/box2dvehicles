...
/*
 * Please see http://www.iforce2d.net/b2djson/ for details on how to use this utility.
 */
    ...
    case e_prismaticJoint:
        {
            ...
            vecToJson("localAxis1", prismaticJoint->GetLocalAxis1(), jointValue);
...
b2World* b2dJson::j2b2World(Json::Value worldValue)
{
    ...
    //if ( ! world->GetAutoClearForces() ) { std::cout << "Warning: world is not set to auto clear forces.\n"; recreationMayDiffer = true; }
    //if ( world->GetWarmStarting() ) { std::cout << "Warning: world is set to use warm starting.\n"; recreationMayDiffer = true; }
    //if ( world->GetContinuousPhysics() ) { std::cout << "Warning: world is set to use continuous physics.\n"; recreationMayDiffer = true; }
    //if ( world->GetSubStepping() ) { std::cout << "Warning: world is set to use sub stepping.\n"; recreationMayDiffer = true; }
    //if ( worldValue["hasDestructionListener"].asBool() ) { std::cout << "Warning: world originally had a destruction listener set.\n"; recreationMayDiffer = true; }
    //if ( worldValue["hasContactFilter"].asBool() ) { std::cout << "Warning: world originally had a contact filter set.\n"; recreationMayDiffer = true; }
    //if ( worldValue["hasContactListener"].asBool() ) { std::cout << "Warning: world originally had a contact listener set.\n"; recreationMayDiffer = true; }
...
b2Fixture* b2dJson::j2b2Fixture(b2Body* body, Json::Value fixtureValue)
{
    ...
    if ( !fixtureValue["circle"].isNull() ) {
		...
		//printf("Created circle fixture¥n");
    }
...
b2Joint* b2dJson::j2b2Joint(b2World* world, Json::Value jointValue)
{
    ...
    else if ( type == "prismatic" )
    {
        ...
        prismaticDef.localAxis1 = jsonToVec("localAxis1", jointValue);
...
int b2dJson::getAllBodyNames(vector<string>& names)
{
...
int b2dJson::getAllFixtureNames(vector<string>& names)
{
...
int b2dJson::getAllJointNames(vector<string>& names)
{
